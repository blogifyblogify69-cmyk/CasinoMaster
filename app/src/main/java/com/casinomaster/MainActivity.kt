...
class MainActivity : Activity() {
        ...
        root.addView(Button(this).apply {
            ...
            setOnClickListener {
                ...
                getSharedPreferences("settings", MODE_PRIVATE).edit()
                    ...
                    .putString("selected_label", selected.label)
                    .apply()
                Toast.makeText(this@MainActivity, selected.label + " added", Toast.LENGTH_SHORT).show()
            }
        })
        root.addView(Button(this).apply {
            text = "OPEN ADDED APP + FLOATING CONTROL"
            setOnClickListener { openSelectedApp() }
        })
        root.addView(Button(this).apply {
        ...
        root.addView(Button(this).apply {
            ...
            setOnClickListener {
                if (!Settings.canDrawOverlays(this@MainActivity)) {
                    ...
                    Toast.makeText(this@MainActivity, "Floating icon permission is enabled.", Toast.LENGTH_SHORT).show()
                }
            }
        })
        root.addView(TextView(this).apply {
    ...
    private fun loadApps() {
        ...
        val selectedPackage = getSharedPreferences("settings", MODE_PRIVATE)
    ...
    private fun openSelectedApp() {
        ...
        if (pkg.isNullOrBlank()) {
            Toast.makeText(this, "Select and add an app first.", Toast.LENGTH_SHORT).show()
            return
        }
        if (!Settings.canDrawOverlays(this)) {
            Toast.makeText(this, "Allow the floating icon permission first.", Toast.LENGTH_LONG).show()
            return
        }

        val launchIntent = packageManager.getLaunchIntentForPackage(pkg)
        if (launchIntent == null) {
            Toast.makeText(this, "The selected app cannot be opened.", Toast.LENGTH_SHORT).show()
            return
        }

        try {
            startForegroundService(
                Intent(this, AppControllerService::class.java).apply {
                    action = AppControllerService.ACTION_SHOW
                }
            )
            ...
            Toast.makeText(
                this,
                "Could not start floating controller: ${e.javaClass.simpleName}",
                Toast.LENGTH_LONG
            ).show()
        }
    }
